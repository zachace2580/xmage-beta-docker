...
java -Xms8G -Xmx16G -server -Djava.security.policy=./config/security.policy -XX:ActiveProcessorCount=4 -Djava.util.logging.config.file=./config/logging.config  -Dsun.java2d.opengl=true -Dlog4j.configuration=file:./config/log4j.properties -jar ./lib/mage-server-*.jar
